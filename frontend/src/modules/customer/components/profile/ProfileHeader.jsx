        ...
        <div className="relative mb-6">
            ...
            <div className="relative pt-16 px-6 flex flex-col items-center">
                ...
                <div className="relative mb-4 group scale-110">
                    <div className="w-24 h-24 rounded-[2rem] border-4 border-white shadow-2xl overflow-hidden bg-gray-50 flex items-center justify-center transform group-hover:rotate-3 transition-transform duration-300">
                        {user?.profileImage && user.profileImage !== 'default_profile.png' ? (
                            ...
                            <span className="text-3xl font-black text-[#FF5C8A] italic">{user?.name?.charAt(0) || 'U'}</span>
                        )}
                    </div>
                    <Link to="/profile/edit" className="absolute -bottom-1 -right-1 bg-gray-900 text-white p-2 rounded-xl shadow-lg hover:bg-[#FF5C8A] transition-all transform hover:scale-110">
                        <Camera size={14} />
                    </Link>
                </div>
                ...
                <div className="flex items-center gap-2">
                    ...
                    <span className="px-3 py-1 bg-pink-50 text-[#FF5C8A] text-[9px] font-black uppercase tracking-[0.2em] rounded-full border border-pink-100 italic">
                        Elite Member
                    </span>
                </div>
                ...
                <div className="grid grid-cols-3 gap-3 w-full max-w-sm mt-8 pb-4">
                    <div className="bg-white p-4 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 text-center group hover:border-[#FF5C8A]/20 transition-all">
                        <span className="block text-xl font-black text-gray-900 group-hover:text-[#FF5C8A] transition-colors">{stats?.totalOrders || 0}</span>
                        <span className="text-[9px] text-gray-400 uppercase font-black tracking-widest leading-none mt-1">Orders</span>
                    </div>
                    <div className="bg-white p-4 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 text-center group hover:border-[#FF5C8A]/20 transition-all">
                        <span className="block text-xl font-black text-gray-900 group-hover:text-[#FF5C8A] transition-colors">{stats?.pendingOrders || 0}</span>
